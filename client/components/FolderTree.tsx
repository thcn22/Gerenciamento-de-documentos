...
import AccessControlModal, { SharedEntry } from "@/components/AccessControlModal";
...
export default function FolderTree({
  ...
  const [accessTargetFolder, setAccessTargetFolder] = useState<FolderNode | null>(null);
  const [folderSharedUsers, setFolderSharedUsers] = useState<SharedEntry[]>([]);
  ...
  const getContextMenuItems = (folder: FolderNode) => [
    ...
    },
    {
      ...
      // Visible only to admin or owner; ContextMenu will check adminOnly flag
      adminOnly: !(isAdmin || (user?.email && folder.createdBy === user.email)),
      onClick: () => {
        setAccessTargetFolder(folder);
        // TODO: load shared users from API if available
        setFolderSharedUsers([]);
  ...
  <div className="flex-1 overflow-y-auto p-2 space-y-1 hide-scrollbar">
      ...
      <ContextMenu
        ...
        isAdmin={
          Boolean(isAdmin) ||
          Boolean(user?.email && selectedFolder && selectedFolder.createdBy === user.email)
        }
      />

      {/* Access control modal for folders */}
      {accessTargetFolder && (
        <AccessControlModal
          isOpen={showAccessModal}
          onClose={() => {
            setShowAccessModal(false);
            setAccessTargetFolder(null);
          }}
          folderId={accessTargetFolder.id}
          folderName={accessTargetFolder.name}
          initialSharedUsers={folderSharedUsers}
        />
      )}
