...
import { checkDocumentPermission } from "../services/permissions";
...
router.get("/documents", authenticateToken, (req: Request, res: Response) => {
  // List documents: filter by permission (non-admin users see only docs they can view)
  const all = DocumentDB.all() as any[];
  if (req.user?.isAdmin) return res.json({ documents: all });
  const filtered = all.filter(d => {
    // keep documents where any folder grants view permission; we'll do a synchronous check via DocumentFolderDB
    const folders = DocumentFolderDB.foldersForDocument(d.id) as string[];
    for (const fid of folders) {
      // Note: checkDocumentPermission is async; to keep this endpoint simple we'll allow listing all and let client request metadata/download which is protected
      // For now, only return documents whose primary folder is accessible
      if (fid === (d.folderId || 'root')) return true;
    }
    return false;
  });
...
router.get("/documents/:id", authenticateToken, async (req: Request, res: Response) => {
  ...
  const ok = await checkDocumentPermission(req.user, id, 'view');
  if (!ok && !req.user?.isAdmin) return res.status(403).json({ error: 'Acesso negado' });
  res.json({ document });
});

// Download/view document
router.get("/documents/:id/download", authenticateTokenAllowQuery, async (req: Request, res: Response) => {
  const { id } = req.params;
  const document: any = DocumentDB.byId(id);
...
router.get("/documents/:id/view", authenticateTokenAllowQuery, async (req: Request, res: Response) => {
  const { id } = req.params;
  const document: any = DocumentDB.byId(id);
