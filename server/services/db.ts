...
-- Folder permissions: subjectType = 'user' | 'group', identifier = email or groupId, permission = 'view'|'edit'|'admin'
...
export const FolderPermissionDB = {
  allForFolder: (folderId: string) => db.prepare('SELECT * FROM folder_permissions WHERE folderId = ?').all(folderId),
  byId: (id: string) => db.prepare('SELECT * FROM folder_permissions WHERE id = ?').get(id),
  find: (folderId: string, subjectType: string, identifier: string) => db.prepare('SELECT * FROM folder_permissions WHERE folderId = ? AND subjectType = ? AND identifier = ?').get(folderId, subjectType, identifier),
  insert: (p: any) => db.prepare('INSERT INTO folder_permissions (id, folderId, subjectType, identifier, permission, createdAt, createdBy) VALUES (@id,@folderId,@subjectType,@identifier,@permission,@createdAt,@createdBy)').run(p),
  update: (p: any) => db.prepare('UPDATE folder_permissions SET permission=@permission WHERE id=@id').run(p),
  removeById: (id: string) => db.prepare('DELETE FROM folder_permissions WHERE id = ?').run(id),
  remove: (folderId: string, subjectType: string, identifier: string) => db.prepare('DELETE FROM folder_permissions WHERE folderId = ? AND subjectType = ? AND identifier = ?').run(folderId, subjectType, identifier),
  removeForFolder: (folderId: string) => db.prepare('DELETE FROM folder_permissions WHERE folderId = ?').run(folderId),
};
